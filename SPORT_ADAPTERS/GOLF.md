...
|---|---|---:|---|---|---|---|---|---|
| [FanDuel Missouri House Rules](https://www.fanduel.com/fanduel-sportsbook-house-rules-mo) plus exact user-visible market/promotion evidence | Missouri promotion terms; golf market labels; minimum odds; standard, OddsBoost, Tourney Special, tie, dead-heat, DNS/WD/DQ, and settlement terms | 1 for FanDuel target display/rules | manual/on-demand rule reference and timestamped screenshot or structured manual entry; no authenticated automation | FanDuel Missouri only | retain rule effective/version date when visible, capture time, local UTC retrieval, raw label, and content hash | reviewed for planning 2026-07-13; exact current rule and product wording must be recaptured before activation | target identity and settlement evidence only | intended target; no automated or recommendation-grade path certified |
| [DraftKings Golf Rules](https://sportsbook.draftkings.com/help/sport-rules/golf) plus exact Missouri terms and user-visible evidence | golf product and settlement reference; exact Missouri market/promotion terms only when separately captured | 1 for exact DraftKings display; general rules are supporting reference | manual/on-demand reference and timestamped screenshot or structured manual entry; no authenticated automation | exact displayed Missouri product only after verification | retain rule/effective date, market title, bet-slip wording, capture time, and local UTC retrieval | general golf rules reviewed for planning 2026-07-13; exact Missouri applicability remains an activation requirement | target identity and settlement evidence only | intended target; Missouri certification absent |
| official organizer, tour, tournament, field, tee-time, leaderboard, rules, and result source | organizer/tour identity, event edition, format, rounds/holes, field/alternates, tee times, course, cut, event status, WD/DQ, shortening, and official finality | 1 for competition facts | manual/on-demand public reference unless the exact terms permit automation; licensed feed when separately approved | named competition/event only | retain publication/effective time, field/version identifier, source URL, and local UTC retrieval | exact organizer terms must be reviewed per source and event | competition identity and material operational facts only | event-specific rule/source pack required |
| [PGA TOUR public pages and terms](https://www.pgatour.com/company/terms-of-use) | PGA TOUR schedule, field, tee-time, leaderboard, cut, WD/DQ, and event-status reference | 1 for applicable PGA TOUR facts | manual/on-demand reference unless separately licensed | applicable PGA TOUR-operated event | retain page/publication state and local UTC retrieval | terms reviewed for planning 2026-07-13; automated retrieval is not approved by this adapter | manual operational verification only | conditional manual reference; no scraper or undocumented endpoint |
| licensed/documented golf-data provider | event, field, tee time, status, cut, participant, course, result, or permitted odds facts | 2 | documented API/feed | contract-defined tours/events/markets | provider update and local UTC timestamps retained | exact contract, wagering-related permission, fields, and transformations must be reviewed | only facts within a separately approved license | provider-specific validation required |
| [National Weather Service API](https://www.weather.gov/documentation/services-web-api) | U.S. venue-matched observations, forecasts, and alerts | 1 for U.S. public weather facts | documented public API with required identifying User-Agent | mapped U.S. venue only | observation/forecast validity and local UTC retrieval required | access guidance reviewed 2026-07-13 | operational delay/suspension gate under a versioned rule; never a probability input | conditional approved source class; venue mapping required |
| official non-U.S. government or organizer operational source | non-U.S. weather or venue operating status | 1 when exact authority and access terms are reviewed | manual/on-demand or permitted documented API | mapped named venue | publication/validity and local UTC retrieval required | source-specific review required | operational gate only | unverified until configured |
| licensed/documented multi-book odds provider | target or comparison outcome sets | 1-2 for quoted markets | documented API/feed | contract-defined books and jurisdictions | provider update plus local request/capture timestamps retained | exact license, golf-market coverage, field behavior, and pricing-origin relationships must be reviewed | exact quotes within approved permission | no provider certified |
| sportsbook-originated comparison evidence | one non-target source market | 2 | timestamped screenshot/structured manual verification or permitted documented feed | exact displayed book/jurisdiction only | local UTC capture required; provider time retained when present | user evidence is conditional; automation requires permission | one complete exact binary or multiway source market | conditional fallback; origin relationship unresolved until configured |
| established secondary reporting | possible field, withdrawal, format, course, or weather lead | 4 | public report | named event/player only | publication and retrieval times required | exact source permission unresolved for automation | may create future `WATCH` pending authoritative confirmation | never recommendation-grade alone |
