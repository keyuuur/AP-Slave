...
|---|---|---:|---|---|---|---|---|---|
| `nba_official_injury_report_2025_26` | season-specific submission policy, report version, and player availability | 1 | registry-owned | league-wide operational facts for the named season | retain publication/version time when available and local UTC retrieval | see registry | operational eligibility and price invalidation only | wrong-season for 2026-27; cannot clear gate |
| `nba_official_schedule` | event identity, date, teams, venue, and scheduled tip | 1 | registry-owned | league-wide event facts | retain event time and local UTC retrieval | see registry | operational event identity only | registry posture controls |
| `source_class_official_team_announcement` | roster identity, transaction, availability, and confirmed role changes | 1-2 | registry-owned | named league/team/event | publication/effective and local UTC retrieval times required | see registry | operational identity, eligibility, and price invalidation | exact instance required |
| `source_class_licensed_sports_data_provider` | schedule, event status, roster, official-report, availability, or confirmed role facts | 2 | registry-owned | contract-defined | provider and local UTC timestamps retained | see registry | operational facts within reviewed license | exact instance required |
| `sportsbook_target_manual_evidence`, `source_class_permitted_sportsbook_feed` | promotion terms and exact target quote | 1 for the displayed target | registry-owned | exact displayed book and jurisdiction only | local UTC capture required; provider time retained if present | see registry | target identity and promotion evidence | manual fallback; feed requires registration |
| `source_class_licensed_multi_book_odds_provider` | exact target or comparison markets | 1-2 for quoted markets | registry-owned | contract-defined books and jurisdictions | provider and local UTC timestamps retained | see registry | exact market evidence within reviewed license | exact instance required |
| `sportsbook_comparison_manual_evidence`, `source_class_permitted_sportsbook_feed` | one exact comparison book | 2 | registry-owned | exact displayed book and jurisdiction | local UTC capture required | see registry | one complete exact comparison pair | manual conditional; feed requires registration |
| `source_class_secondary_reporting_lead` | possible registered material-change lead | 4 | registry-owned | named player/team/event only | publication and retrieval times required | see registry | may create `WATCH` pending authoritative confirmation | never recommendation-grade alone |

The `nba_terms_of_use` registry record preserves the current restriction against using the site's defined NBA Statistics in connection with gambling activity. Therefore NBA.com, stats.nba.com, or other NBA-owned statistical fields must not be collected, stored as gambling-model inputs, scored, narrated as candidate support, or used to change probability or ranking in this workflow. A future Tier D model requires a separately licensed/permitted statistical source and documented legal/license review.
...
Injury-report deadlines are season-specific configuration, never permanent adapter constants. Before any run could clear the structural injury gate, register and reverify `nba_injury_deadlines_[season]_v1` from the official season artifact, including the applicable deadline logic, report/version identity, effective date, source ID, and review timestamp. `nba_official_injury_report_2025_26` is historical and wrong-season for 2026-27; it cannot clear a later-season gate.
